...
import com.gaoan.forever.tablemodel.RoleTableModel;
...
import com.gaoan.forever.window.role.RoleInsertWindow;
import com.gaoan.forever.window.role.RoleUpdateWindow;
import com.github.pagehelper.PageInfo;
import com.google.gson.reflect.TypeToken;

public class RoleTab {

	private static final Logger logger = LoggerFactory.getLogger(RoleTab.class);

	private static final String ROLE_URL = ServerApiConfig.getApiGetRolePage();

	private static final String ROLE_DELETE_URL = ServerApiConfig.getApiDelRole();
	...
	public static JComponent show(JFrame window) {
		...
		// 查询订货单列表
		JTable table = null;
		RoleTableModel tableModel = new RoleTableModel();

		PageInfo<RoleInfoModel> pageInfo = null;

		List<RoleInfoModel> orderList = new ArrayList<RoleInfoModel>();
		// 初始化tableModel
		...
		// 初始化页码栏
		Box bottomBox = ComponentUtils.initBottomBox(pageInfo, ROLE_URL, new OrderQueryConditionModel(), window,
				tableModel, mainBox);
	 ...
	 * @param tableModel
	 */
	private static void addListener(RepaintContentParamVo<RoleInfoModel> paramVo) {
		...
		insertBtn.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				RoleInsertWindow.show(window, paramVo);
			}
		});
		...
		updateBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (row < 0) {
					...
					List<RoleInfoModel> list = ((RoleTableModel) table.getModel()).getData();
					RoleInfoModel selectModel = null;
					if (CollectionUtils.isNotEmpty(list)) {
						selectModel = list.get(row);
					}
					RoleUpdateWindow.show(window, selectModel.getId(), paramVo);
				}
			}
		});
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (row < 0) {
					...
					if (chooseResult == JOptionPane.YES_OPTION) {
						List<RoleInfoModel> list = ((RoleTableModel) table.getModel()).getData();
						RoleInfoModel selectModel = list.get(row);
						Map<String, String> param = new HashMap<String, String>();
						param.put("roleId", selectModel.getId().toString());
						Map<String, Object> result = CallUtils.executeGet(window, ROLE_DELETE_URL, param,
	...
	public static void repaintContentBox(RepaintContentParamVo<RoleInfoModel> paramVo) {
		JFrame window = paramVo.getWindow();
		RoleTableModel tableModel = (RoleTableModel) paramVo.getTableModel();
		Box mainBox = paramVo.getMainBox();

		Map<String, Object> roleMap = CallUtils.post(window, ROLE_URL, GsonUtils.toJson(new JSONObject()),
				MessageEnum.SEARCH_MSG);

		PageInfo<RoleInfoModel> pageInfo = GsonUtils.fromJson(roleMap.get("pageInfo"),
				new TypeToken<PageInfo<RoleInfoModel>>() {
				});
		...
		mainBox.remove(2);
		mainBox.add(ComponentUtils.initBottomBox(pageInfo, ROLE_URL, new HashMap<>(), window, tableModel, mainBox));
		mainBox.repaint();
	}
}
